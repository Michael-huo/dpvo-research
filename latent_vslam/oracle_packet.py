...
def extract_native_packet(slam: Any, image: Any) -> NativeFeaturePacket:
    import torch
    ...
    with torch.no_grad(), torch.cuda.amp.autocast(enabled=bool(slam.cfg.MIXED_PRECISION)):
        fmap, gmap, imap, patches, _, colors = slam.network.patchify(
            normalized, patches_per_image=int(slam.M),
            centroid_sel_strat=slam.cfg.CENTROID_SEL_STRAT, return_color=True,
        )
