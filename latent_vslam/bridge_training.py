"""Feature extraction and Bridge training."""
...
from prediction.jepa_runtime import JepaSidecar, load_dpvo_domain, load_fnet, state_dict_sha256, teacher_fmap
from latent_vslam.oracle_packet import FMapZeroContextPacket, _derive_frontend_state
from latent_vslam.protocol import FrameIdentity, canonical_sha256, repo_path, sha256_file
...
def _evaluate_bridge(
    ...
    for start in range(0, len(teacher_rows), batch_size):
        ...
        with scope:
            tokens = bridge_tensor_batch(store, "block5", source)
            teacher = bridge_tensor_batch(store, "teacher", target)
    ...
    for pass_index in range(int(bridge["passes"])):
        ...
        for local_epoch in range(epochs_per_pass):
            ...
            for rows in batches:
                ...
                scope = contextlib.nullcontext() if profiler is None else profiler.stage(stage)
                with scope:
                    tokens = bridge_tensor_batch(store, "block5", rows)
                    teacher = bridge_tensor_batch(store, "teacher", rows)
