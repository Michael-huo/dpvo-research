"""Bounded delayed prediction pipeline; graph mutations belong exclusively to the consumer."""
...
from latent_vslam.efficiency_profiling import TransferLedger
from latent_vslam.prediction_deployment import DelayedDeploymentProvider
from latent_vslam.jepa_fmap import preprocess_full_fov_rgb
from prediction.jepa_runtime import load_dpvo_domain
from latent_vslam.oracle_packet import FMapZeroContextPacket
from latent_vslam.profiling import distribution_ms
from latent_vslam.protocol import atomic_write_json
from latent_vslam.runtime import AnchorRGBObservation, PacketObservation
from latent_vslam.staged_transfer import GPUWorker, PinnedTransfer, SharedSlot
...
class PredictionPipeline(DelayedDeploymentProvider):
    ...
    def _check(self):
        ...
        if self.stop.is_set():
            raise RuntimeError(f"prediction pipeline cancelled; unconsumed_identity={unconsumed}")
    ...
    def online_session(self):
        ...
        primary_error = None
        try:
            self.workers["encoder"] = GPUWorker(python=sys.executable,
            ...
            if primary_error is not None:
                ...
                raise RuntimeError(f"prediction pipeline lifecycle cleanup failed: {errors}")

    @property
    def predictor_config(self):
        from prediction.predictor import predictor_metadata
