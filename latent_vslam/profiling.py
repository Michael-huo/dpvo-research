"""Pure aggregation helpers for prediction deployment profiling."""
...
class OnlineProfiler:
    ...
    def payload(self, *, peak_online_vram_bytes: int) -> dict[str, Any]:
        ...
        return {
            "schema": "prediction_stage_profile_v2",
            ...
            "stage_c_timing": {
                "schema": "prediction_stage_c_timing_v1",
...
def matched_wall_clock_payload(*, full_rgb_seconds: float, ours_seconds: float,
                               sparse_rgb_seconds: float) -> dict[str, Any]:
    values = {
        "full_rgb_total_s": float(full_rgb_seconds),
        "ours_total_s": float(ours_seconds),
    ...
    values["ours_over_full_rgb_ratio"] = values["ours_total_s"] / full if full > 0 else None
    values["extra_cloud_compute_s"] = values["ours_total_s"] - full
