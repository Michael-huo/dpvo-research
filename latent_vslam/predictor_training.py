"""Training and representation diagnostics for the predictor."""
...
from latent_vslam.dpvo_backend import _seed_everything
from latent_vslam.jepa_fmap import coordinate_masks, tokens_to_field
from prediction.jepa_runtime import CompactFeatureStore
from latent_vslam.oracle_packet import FMapZeroContextPacket, _derive_frontend_state
from prediction.predictor import AnchorInterval, RobustTransportBlock5Predictor, prediction_loss
from latent_vslam.protocol import FrameIdentity, canonical_sha256
from prediction.transport import (
...
def held_out_representation(split_test: Sequence[AnchorInterval], store: CompactFeatureStore,
    ...
    return {"evaluation_role": "held_out_mh01_representation_test",
            ...
            "fmap_target": "offline_true_fmap_teacher",
            "gate2_frozen_bridge_vs_true_fmap": {
...
def _plot_feature_diagnostics(path: Path, sequence: str, intervals: Sequence[AnchorInterval],
    ...
    error_limits={"jepa_prediction":vmax(jepa_error),"bridge_to_true_fmap":vmax(h1_error),
                  "prediction_through_bridge":vmax(h2_error)}
    ...
    for row_index,metadata in enumerate(sample_metadata):
        ...
        for column,value in enumerate(images):
            if column in (2,6,7):
                key=("jepa_prediction","bridge_to_true_fmap","prediction_through_bridge")[(2,6,7).index(column)]
