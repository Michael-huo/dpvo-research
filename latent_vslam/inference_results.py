"""Inference stride sweep tables and shared trajectory references."""
...
from latent_vslam.evaluation import (dense_hidden_ate, evaluate_paired_trajectory,
                         filter_groundtruth_associable_timestamps,
                         freeze_evaluation_population, population_coverage)
from latent_vslam.profiling import graph_workload_payload
from latent_vslam.protocol import atomic_write_bytes, atomic_write_json, repo_path, sha256_file
from latent_vslam.inference_runtime import _compact_runtime
...
def save_trajectory(output, row, records, roles, config, *, dense):
    ...
    if admitted:
        from latent_vslam.evaluation import admitted_native_evaluation
    ...
    if "online_profile" in row:
        result["prediction_stage_profile"] = row["online_profile"]
...
def comparison_row(population, sparse, ours, training):
    ...
    profile = ours["prediction_stage_profile"]
    return {
...
def write_tables(root, index):
    ...
    lines = ["# Inference stride sweep — Communication sensitivity", "",
