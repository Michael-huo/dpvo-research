...
from latent_vslam.dpvo_backend import _load_frame, _make_slam, _runtime_classes, _seed_everything
from latent_vslam.oracle_packet import (
    FrontendPacketStore,
    ZERO_PACKET_SCHEMA,
...
def run_packet_observations(
    ...
    The iterable may be lazy: inference uses this to make the closing anchor arrive and
...
def run_deployment_observations(
    ...
    """Run strict prediction with native RGB anchors and FMap-only hidden packets."""
    if image_height <= 0 or image_width <= 0:
        raise ValueError("positive DPVO image geometry is required")
    ...
    from latent_vslam.uniform_admission import AdmissionReceipt
    from latent_vslam.observation_sampling import observation_rng_scope, SAMPLING_PROTOCOL
