...
from .profiling import (OnlineProfiler, break_even_payload, context_wait_values,
                        distribution_ms)
from .protocol import FrameIdentity, atomic_write_json, post_bootstrap_ratio_roles
from .run_h2 import (CONDITIONS, TRAINING_ANCHOR_RATIO, _SUMMARY_CONDITIONS,
                     _metadata, _write_sequence_summary, load_config)
...
class H2ContractTest(unittest.TestCase):
    def _summary_results(self) -> dict:
        values = {
            ...
            for index, (key, _label) in enumerate(_SUMMARY_CONDITIONS)
        }
        conditions = {}
        ...
        return {
            ...
            "protocol": {"timestamp_causal": False},
            "training": {"must_not_appear": True},
            "provenance": {"sha256": "must_not_appear"},
            "sequences": {
                "MH_01_easy": {
                    "candidate_count": 101,
                    "hidden_count": 78,
    ...
    def test_delayed_replay_encodes_only_anchors_and_consumes_hidden_once(self) -> None:
        ...
        with patch(
            "research.src.phase1_dpvo_feasibility.exp6.h2_deployment.JepaSidecar",
            FakeSidecar,
    ...
    def test_sequence_summary_matches_canonical_results_without_rounding(self) -> None:
        results = self._summary_results()
        with tempfile.TemporaryDirectory() as name:
            results_path = Path(name) / "results.json"
            summary_path = Path(name) / "MH_01_easy" / "SUMMARY.md"
            atomic_write_json(results_path, results)
            _write_sequence_summary(summary_path, results, "MH_01_easy")
            persisted_results = json.loads(results_path.read_text(encoding="utf-8"))
            summary = summary_path.read_text(encoding="utf-8")
        sequence_result = persisted_results["sequences"]["MH_01_easy"]
        lines = summary.splitlines()
        self.assertEqual(lines[2], (
            "| Condition | ATE RMSE (m) | translation RPE@1s (m) | "
            "rotation RPE@1s (deg) | coverage | final node count |"
        ))

        table_rows = lines[4:9]
        self.assertEqual(
            [row.split("|")[1].strip() for row in table_rows],
            [label for _key, label in _SUMMARY_CONDITIONS],
        )
        for row, (key, _label) in zip(table_rows, _SUMMARY_CONDITIONS):
            cells = [cell.strip() for cell in row.split("|")[1:-1]]
            condition = sequence_result["conditions"][key]
            evaluation = condition["canonical_evaluation"]
            self.assertEqual(float(cells[1]), evaluation["ate_rmse_m"])
            self.assertEqual(float(cells[2]), evaluation["translation_rpe_rmse_m"])
            self.assertEqual(float(cells[3]), evaluation["rotation_rpe_rmse_deg"])
            self.assertEqual(
                float(cells[4]),
                condition["canonical_coverage"]["canonical_pose_coverage"],
            )
            self.assertEqual(
                int(cells[5]),
                condition["runtime"]["final_node_count_before_terminate"],
            )
        ...
        predicted = sequence_result["conditions"]["predicted_jepa_hidden"]["runtime"]
        self.assertEqual(float(metadata["Anchor ratio"]),
                         sequence_result["schedule"]["actual_full_sequence_anchor_ratio"])
        uploaded, candidates = metadata["Uploaded anchor count / total candidates"].split(" / ")
        self.assertEqual(int(uploaded), predicted["rgb_uploaded_frame_count"])
        self.assertEqual(int(candidates), sequence_result["candidate_count"])
        self.assertEqual(int(metadata["Hidden count"]), sequence_result["hidden_count"])
    ...
    def test_sequence_summary_is_atomic_and_contains_no_low_frequency_details(self) -> None:
        results = self._summary_results()
        with tempfile.TemporaryDirectory() as name:
            output = Path(name) / "SUMMARY.md"
            with patch.object(Path, "read_text", side_effect=AssertionError("unexpected read")):
                _write_sequence_summary(output, results, "MH_01_easy")
            summary = output.read_text(encoding="utf-8")
        lowered = summary.lower()
        for forbidden in ("sim3", "sha256", "training", "profiling"):
            self.assertNotIn(forbidden, lowered)
        self.assertIn("root `results.json` remains the complete, authoritative data source", summary)

    def test_h2_does_not_import_h1_runner_or_results(self) -> None:
        module = __import__("research.src.phase1_dpvo_feasibility.exp6.run_h2", fromlist=["run"])
