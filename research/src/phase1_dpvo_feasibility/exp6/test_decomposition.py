...
from .run_decomposition import CONDITIONS, load_config
...
class DecompositionProtocolTest(unittest.TestCase):
    ...
    def test_final_conditions_and_config(self) -> None:
        ...
        source = inspect.getsource(__import__(
            "research.src.phase1_dpvo_feasibility.exp6.run_decomposition",
            fromlist=["run"],
        ))
    ...
    def test_only_three_public_exp6_modules_and_configs_remain(self) -> None:
        source_root = Path(__file__).parent
        runners = {path.name for path in source_root.glob("run_*.py")}
        self.assertEqual(runners, {"run_decomposition.py", "run_h1.py", "run_h2.py"})
        config_root = source_root.parents[2] / "configs"
        configs = {path.name for path in config_root.glob("*exp6*.yaml")}
        self.assertEqual(configs, {"exp6_decomposition.yaml", "exp6_h1.yaml", "exp6_h2.yaml"})
